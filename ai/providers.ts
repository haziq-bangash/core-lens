...
export function canUseModel(modelValue: string, user: any, _isProUser?: boolean): { canUse: boolean; reason?: string } {
  const model = getModelConfig(modelValue);
...
export function getAcceptedFileTypes(modelValue: string, _isProUser?: boolean): string {
  const model = getModelConfig(modelValue);
  if (model?.pdf) {
    return 'image/*,.pdf';
  }
